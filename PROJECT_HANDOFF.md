...
**Branch in progress:** **Per-Event Template System** — Stage 0 + Stage 1a done + live (PR #83); **BLOCKED on Mohanad providing the concrete second-event identity** for template #2 (gates the Stage-2 mockup). Stage 1b (shared-engine factoring) recommended to fold into Stage 2. Other open follow-ups: FILE-field has-file/no-file filtering; optional mobile-responsive field widths (THIRD/HALF stack on phones); Section-Headers v2 non-goals (collapsible / conditional-empty-hide / descriptions); abandoned-branding-upload orphans left unswept by design.
...
### 2026-06-29 — Per-Event Template System — Stage 0 + Stage 1a — live on prod (project STARTED)
...
**Stage 1b (NEXT) + Stage 2 — status / recommendation.** Stage 1b = factor the shared `<RegistrationFormBody>` engine (state hook + `renderField` + stepper) out of Classic so template #2 imports it and can't fork it (§3). After seeing the real surface (~1,130 intertwined lines), the recommendation is to **fold 1b into Stage 2** — you can't verify a "shared" engine with only one consumer, and the system already functions whole. **Stage 2 (build template #2) is BLOCKED pending the concrete second-event identity** — per the spec the mockup can't start in the abstract. v1 scope = infra + Classic + exactly ONE new template; non-goals (held): JSON page builder, per-template data models, N templates up front, admin knob auto-hiding, per-template thumbnails.
...
_**Active project: Per-Event Template System** — Stage 0 + Stage 1a done + live (PR #83, see top entry). **BLOCKED on Mohanad providing the concrete second-event identity** for template #2 (gates the Stage-2 mockup; the spec forbids starting it in the abstract). Stage 1b (shared-engine factoring) recommended to fold into Stage 2. No other queued substantive feature._
