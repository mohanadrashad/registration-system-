...
**Last updated:** 2026-06-29 (**Per-Event Template System — STARTED. Stage 0 (audit) + Stage 1a (registry + extract Classic) DONE + live + prod-verified, PR #83.** Stage 0 resolved: `EventBranding` is nullable AND not auto-created → `template` lives on **`Event`** (`RegistrationTemplate @default(CLASSIC)`, additive, no backfill); the public renderer has NO per-event code branching (only `customCss` data) so extraction is clean; the load-bearing customCss hooks (`data-event-*`, `.registration-form`, `.submit-button`) are the one byte-identical landmine. Stage 1a shipped the curated component-per-template **registry** + `ClassicTemplate` (today's renderer moved **byte-for-byte**) + a thin container that dispatches via a lightweight `/api/register/[slug]/template` endpoint; **byte-identical gate PASSED** — before/after Playwright pixel-diff = 0 differing pixels (only the prod-absent Next dev overlay differed). **Template #2 PARKED — deliberate decision (2026-06-29), not a blocker.** No concrete second design exists; "max customization / pro" is a goal, not a design, and a curated template needs a real structural target — a near-clone of Classic would waste the v1 slot and wouldn't validate the shared engine. Infra (1a) stays in prod as-is; **NO Stage 1b / Stage 2** until a real event with a genuinely different structure supplies a design brief (the seam-factoring stays folded into that future Stage 2). Prior: three debt items cleared: (1) Excel-export duplicate role-mapped column — already fixed by #66, verified; (2) **friendlier admin file-op errors** — pre-flight surfaces the real reason (expired session / empty-field guard) the @vercel/blob SDK hides, PR #80 live; (3) **branding reconcile-on-replace** — saving a new logo/favicon over an old one now `del()`s the orphaned `branding-public` blob (guarded to our own blobs, best-effort), PR #81 live; no cron for abandoned-upload orphans (agreed). Prior 2026-06-29: two deferred-redesign threads — **THIRD-width defect FIXED** — the form grid was `grid-cols-2` so `FieldWidth.THIRD` mapped to `col-span-1` = 50% (identical to HALF, 3-across impossible); moved to a 6-col grid (FULL=6, HALF=3, THIRD=2), HALF/FULL unchanged, browser-render-verified (3-across THIRDs, THIRD-first-in-section, HALF+THIRD mix), PR #78 live. **footerText** confirmed **already rendering** RTL-aware (`page.tsx:1380`) — not inert, left untouched. Prior 2026-06-29: **Section Group Headers — FEATURE COMPLETE (v1)**, PRs #74–#76, all live. Stage 0 audit found the mechanism was already built: `HEADING` field type groups the public form positionally; `FormField.section` is dead (never read, no admin UI). Shipped: §5 polished section-header look (small-uppercase muted label + thin flat rule) on the register page AND portal phases via shared `<SectionHeading>` (#74); **per-heading label color** picked in the form builder, stored in `FormField.metadata.color` — NO schema change (#75); §6 builder polish — distinct dashed heading rows + color swatch + "Add section heading" button (#76). Earlier 2026-06-29: Bug fix — **attendee-edit 400**: the Edit-attendee save re-sends the contact's STORED email + category, so any contact whose stored value no longer passes current validation (a category since removed from the event's list, or a legacy-format email) was **un-editable** — every save 400'd, even fixing a name. The PUT now grandfathers an UNCHANGED value (validates only a changed email/category); PR #72, live. Prior window (2026-06-28): Attendee-list **quality-of-life arc** — 6 PRs #65–#70, all live + prod-verified: La Gloire favicon + Open Graph link-preview (#65); export field-mapping dedup that also closed the long-logged company→ORGANIZATION double-column (#66); clickable Website/Social + file links in the attendees list (#67); **per-event sequential attendee ID** — `Contact.serialNumber`, additive schema + race-safe allocation + idempotent bulk backfill (#68); wider Name column (#69); **filter-by-absence** — Uncategorized / ungrouped / no-answer via one shared sentinel in the shared where-builder so screen==export (#70). Earlier (2026-06-22): Attendees **column manager** COMPLETE (PRs #59–#63); Attendee Groups (#55–#58); attendees dynamic filters + server pagination (#53))
**Owner:** Mohanad
...
**Branch in progress:** none — between projects. Per-Event Template System **infra shipped (Stage 1a, live, PR #83)**; **template #2 PARKED by decision (2026-06-29)** pending a concrete design brief — no Stage 1b / Stage 2 until a real event with a genuinely different structure exists (seam-factoring folded into that future Stage 2). Other open follow-ups: FILE-field has-file/no-file filtering; optional mobile-responsive field widths (THIRD/HALF stack on phones); Section-Headers v2 non-goals (collapsible / conditional-empty-hide / descriptions); abandoned-branding-upload orphans left unswept by design.
**Production branch:** `main`, HEAD at `52c95a4` (template system Stage 1a, PR #83)
...
### 2026-06-29 — Per-Event Template System — Stage 0 + Stage 1a live on prod; template #2 PARKED by decision

The biggest, highest-regression-risk project to date: let each event render through a distinct curated **template** (its own layout/identity) while every template shares one form/data engine. Spec: `Per-Event Template / Layout System — Feature Spec`. Curated **component-per-template registry** (NOT a config/JSON page builder); the inviolable §3 boundary is **templates own chrome, the field/stepper/validation/submit engine is shared**. Multi-session; staged hard.

**Stage 0 — audit (against real code). Resolved the spec's open questions:**
- **`EventBranding` is nullable AND not auto-created** (the only write is the admin branding upsert). So `template` lives on **`Event`**, not EventBranding — the field deciding which renderer runs must always resolve. (Decision deferred to me by Mohanad with "max customize + pro system"; Event-level is the robust choice.)
- **No per-event code branching** in the public renderer — the only per-event variation is `branding.customCss` (data, injected as a `<style>`). So extraction is structurally clean. **The one byte-identical landmine:** customCss (Productive Families) targets load-bearing hooks `data-event-date/time/venue`, `.registration-form`, `.submit-button` — the Classic template MUST preserve them.
- **Renderer = `(public)/register/[eventSlug]/page.tsx`**, a ~1385-line `"use client"` component that fetches `/api/register/[slug]` → `EventData`, owns all form state, renders only the single REGISTRATION phase's steps (POST_REG phases are the portal, a separate surface). The §3 seam: shared engine = state + `renderField` (FieldWidth 6-col grid, HEADING grouping, conditional) + stepper + validation + submit; chrome = card shell / header / welcome / footer / where the form sits.

**Stage 1a — registry + extract Classic — `52c95a4` (PR #83, squash merge), live + prod-verified.** No new look.
- schema: `enum RegistrationTemplate { CLASSIC }` + `Event.template @default(CLASSIC)` (additive, no backfill; **maintainer ran the prod `db push`** — no `--accept-data-loss` needed).
- `src/components/register-templates/`: `classic-template.tsx` (the renderer moved **byte-for-byte**, only the export name changed), `registry.ts` (`TEMPLATE_REGISTRY` + `resolveTemplate`, unknown→CLASSIC), `types.ts`.
- `api/register/[eventSlug]/template/route.ts`: lightweight template resolver the container dispatches on (retires when Stage 1b's shared hook owns the single fetch).
- `page.tsx` → thin container: resolves `Event.template` and renders the chosen template; null+CLASSIC both → ClassicTemplate (stable ref → mounts once, no flash).
- **Byte-identical gate PASSED (the spec's required diff, not eyeballing):** before/after Playwright pixel-diff on a rich throwaway event (logo header, section heading, FULL/HALF/THIRD widths, footer, customCss hitting all 3 hooks) → **0 differing pixels** once the prod-absent Next dev "Rendering…" overlay is masked. Prod-verified: `/api/register/productive-families/template` → `{"template":"CLASSIC"}`, register page + full API both 200.

**Stage 1b + Stage 2 — PARKED by decision (2026-06-29).** Stage 1b = factor the shared `<RegistrationFormBody>` engine (state hook + `renderField` + stepper) out of Classic so template #2 imports it and can't fork it (§3). After seeing the real surface (~1,130 intertwined lines), it stays **folded into Stage 2** — you can't verify a "shared" engine with only one consumer, and the system already functions whole. **Template #2 is parked, not merely blocked:** there is no concrete second design — "max customization / pro" is a goal, not a design; a near-clone of Classic would waste the v1 slot and wouldn't validate the shared engine. **Resume trigger:** a real event with a genuinely different visual structure + a design brief to design against. v1 scope = infra + Classic + exactly ONE new template; non-goals (held): JSON page builder, per-template data models, N templates up front, admin knob auto-hiding, per-template thumbnails. (The curated-vs-builder question was raised + settled: curated components give "genuinely different structures"; a builder gives only rearranged blocks — and per-event branding knobs + customCss already cover in-template customization. The Stage-1a registry could later host one "Custom/Builder" template too, with nothing redone.)

### 2026-06-29 — Three debt items cleared (export dedup verify, admin file-op errors, branding reconcile-on-replace) — live on prod

- **Excel-export duplicate role-mapped column — already fixed, verified (no PR).** The PR #48 out-of-scope note (a field mapped via `mapsTo` but named differently, e.g. `company`→ORGANIZATION, rendering as BOTH a base column AND a dynamic column) was already closed by **PR #66** this window (`isDynamicFormField` excludes by `mapsTo`, not just `name`). Re-verified: a `company`→ORGANIZATION field produces **exactly one** column (the Organization base column); unmapped fields still appear as dynamic columns.
- **Friendlier admin file-op errors (`e42ffd1`, PR #80).** `@vercel/blob` `upload()` throws a generic "Failed to retrieve the client token" when the server's `onBeforeGenerateToken` rejects (its 400 body is swallowed), hiding the reason. Added `preflightFileOp` in `file-field-edit-cell.tsx`: a cheap GET to the **existing** read-back endpoint (`.../fields/[formFieldId]/file`, same `authorizeEvent`) before `upload()`, surfacing the two hidden reasons — **expired session** (401/403) and the **empty-field guard** (a file already exists → "use Replace instead"). Best-effort (falls through on network blips); the amber "taking longer" timeout state is untouched. Size/MIME rejections already surface decent SDK messages (client-side token validation), so they weren't the generic-message case. `FormFieldDef` has no metadata, so client-side size/MIME pre-checks were intentionally NOT added (would need plumbing).
- **Branding reconcile-on-replace (`be6600d`, PR #81).** Finding (reported first): the branding save (`POST /branding`) just overwrote `logoUrl`/`logoWhiteUrl`/`faviconUrl`; **no `del()` existed anywhere** → replacing an uploaded image orphaned the old blob in `branding-public`. Added: snapshot the current image URLs before the upsert, and after it `del()` any that this save **replaced**. Guards: only our own public-store blobs (`*.public.blob.vercel-storage.com` — pasted external URLs untouched), only when the URL changed (never an in-use blob), best-effort try/catch with `BLOB_PUBLIC_READ_WRITE_TOKEN` (a failed delete never fails the save). **No cron sweep** for abandoned uploads (uploaded then never saved) — a KB-scale orphan whose deletion footgun isn't worth it (agreed with the debt note). Memory `[[blob-stores-and-dev-db-footgun]]`.

### 2026-06-29 — Two deferred-redesign threads resolved (THIRD-width fix + footerText status) — live on prod

Both were public-register-renderer status questions from the deferred-redesign list. Stage-0-style: read current code, report, act.

- **THIRD-width — DEFECT, FIXED (`2b16e2f`, PR #78).** Mohanad's earlier report was real. The form grid (`page.tsx`) was `grid grid-cols-2`, and the width map sent `HALF → col-span-1` AND `THIRD → col-span-1` — so **THIRD rendered at 50%, identical to HALF**; 3-across was impossible. Fix: a **6-column grid** (LCM of 2 and 3) — `FULL=col-span-6`, `HALF=col-span-3` (unchanged 50%), `THIRD=col-span-2` (33%); the full-row layout elements (section heading / divider / paragraph) went `col-span-2 → col-span-6`. HALF/FULL proportions preserved exactly; only THIRD changed (broken → correct). **This closes the "open THIRD-width verdict" the Section Group Headers spec flagged (§3.5).**
  - **Verified with a real browser render** (Playwright + Chrome against `npm run dev`, throwaway event): measured **3× THIRD at ~33% sharing one row** — including a **THIRD first-in-section** directly after a section heading (grid auto-placement flows it correctly), a **HALF (49%) + THIRD (33%)** mix on its own row, and **FULL = 100%**. Screenshot confirmed visually. `tsc` clean, Preview green before merge.
  - **Known characteristic (not a bug):** widths are **non-responsive** — THIRD is 33% on all screen sizes (matching the existing grid, where HALF is already 50% on mobile). Deliberately kept that way to avoid regressing HALF's mobile behavior. A future enhancement could stack fields full-width on phones (`sm:` breakpoints) — logged as an open follow-up, not done unasked.
- **footerText — already renders, NOT inert (no action).** `EventBranding.footerText`/`footerTextAr` resolve RTL-aware at `page.tsx:555` (`footerTextAr` in Arabic, else `footerText`, with fallbacks) and render at `page.tsx:1380` as `<p className="text-center text-xs text-gray-400 mt-8">` below the form in the card (form view, not success view — reasonable). Per the task rules, reported only — no wiring needed.

### 2026-06-29 — Section Group Headers — FEATURE COMPLETE (v1) — live on prod

Labelled visual sections on public forms ("Personal Info", "Work Details") so a long registration isn't one flat wall of fields. Spec: `Section Group Headers — Feature Spec`. Three PRs (#74–#76), all squash-merged + deploy-verified.

**Stage 0 audit — the core fork resolved: the mechanism was already built.** Run against current code (the spec's gating step). `HEADING` is the live grouping mechanism: an admin adds a "Section Heading" field (Form Builder → Add Field → type "Section Heading", in `FIELD_TYPE_LABELS`), it renders on the public page (was a plain `<h3>`) and groups the fields after it — bilingual (`label`/`labelAr` via `getFieldLabel`), **within a step** (each step renders its own `activeStep.fields`, so a section never spans a step by construction), width-correct (`col-span-2` heading → following HALF/THIRD flows into the next grid row), ordered by `FormField.order`. **`FormField.section` is dead**: plumbed through the API + register DTO but **never read by the renderer and no admin UI sets it** — a legacy/abandoned attempt, exactly as the spec hypothesized. So no parallel mechanism was introduced and **no schema change** was needed.

- **PR #74 (`cd4d914`) — §5 renderer restyle.** New shared `src/components/public/section-heading.tsx` `<SectionHeading>`: small-uppercase muted label + a thin **flat** 1px rule beneath (no gradient/glow — those flicker on render) + `mt-6 first:mt-0` spacing. Replaces the plain bold `<h3>` in **both** the public register page AND the post-registration **portal phases** (user-chosen scope), so the look is consistent. RTL handled by the caller's `dir`; uppercase is a no-op on Arabic.
- **PR #75 (`1b25f72`) — per-heading label color, picked in the form builder.** The admin's idea ("why not color-pick when I add the section heading?"). A color picker shown only for HEADING in the Add + Edit dialogs (`HeadingColorField`; Reset → default muted gray). Stored in **`FormField.metadata.color`** — the metadata Json column already exists and the form-fields routes already pass non-FILE metadata through, so **NO schema change / no DB step**. New `src/lib/form-builder/heading-meta.ts` `parseHeadingColor()` reads a valid 6-digit hex **defensively** (garbage → default, never breaks the page); used by both public renderers. The **portal phase DTO** now forwards `FormField.metadata` (it was selected but dropped at the map). Label text only takes the color; the rule stays gray. Verified with `parseHeadingColor` unit cases + a real Prisma Json round-trip on a HEADING field.
- **PR #76 (`f7898e3`) — §6 form-builder polish.** HEADING rows in the field list render distinct from field cards (dashed border + muted tint) with a small **swatch** showing the section's chosen color; new **"Add section heading"** button beside "Add Field" (opens the Add dialog pre-set to HEADING). Add/edit/reorder/delete already worked (a heading is a normal draggable field row).

**How to use it:** the new look appears wherever a form *has* a Section Heading; forms with none are unchanged until an admin adds one. **v2 non-goals (explicitly deferred):** collapsible sections; header auto-hide when its section is conditionally empty; section descriptions/help text.
...
Six small-to-mid PRs (#65–#70), all squash-merged to `main` and prod-verified. Mostly attendee-list polish driven by maintainer reports while reviewing the productive-families list. No DB step except #68 (one additive column + a backfill the maintainer ran against prod).

- **PR #65 (`56e7a45`) — La Gloire favicon + Open Graph link-preview.** Replaced the stock create-next-app `favicon.ico` (the "Vercel-looking" mark that showed in browser tabs AND when a `/register/...` link was pasted into WhatsApp/Slack) with a La Gloire icon, and added OG + Twitter metadata so the large link-preview card shows the brand. New `src/app/favicon.ico` (multi-size **PNG-in-ICO**, white "LG" on brand green — generated with `sharp`, which **can't write .ico** so the ICO container is hand-assembled), `src/app/icon.png` (512) + `apple-icon.png` (180) auto-detected by Next, `public/og-image.png` (1200×630, wordmark on the dark `#0c0c0e` header color + green→magenta accent bar). `layout.tsx` gained `metadataBase` (mirrors `getFallbackAppUrl()` precedence: `APP_URL → NEXT_PUBLIC_APP_URL → localhost`) + openGraph/twitter — **inherited by the client-component registration page** (a client page can't export `metadata`; the root layout's applies). og:image resolves to the real domain in prod because `APP_URL` is set in Vercel. **Dead feature noted:** the per-event `branding.faviconUrl` upload field is stored but **never rendered** into the public `<head>` — that's the wiring point if per-event favicons are ever wanted.
- **PR #66 (`725e0b6`) — export: drop field-mapped name fields from dynamic columns.** Closes the **long-logged `company→ORGANIZATION` duplication quirk** (handoff 2026-06-08). First/Last Name appeared **twice** in the registrations export (pinned base column + a dynamic form-answer column) because `isDynamicFormField` deduped only by machine `name` (`firstName`/`lastName`), missing fields that feed a base column via `mapsTo` (FIRST_NAME/LAST_NAME/FULL_NAME/EMAIL/PHONE/ORGANIZATION/DESIGNATION). Now `isDynamicFormField` also excludes any field with a non-null `mapsTo`. Fixed in the **shared** `format-form-value.ts` so the attendees screen AND export stay consistent; `attendees/route.ts` now selects `mapsTo` (export route already did). 9-case unit test.
- **PR #67 (`6881f17`) — clickable Website/Social + file links in the list.** Admins no longer open each profile to follow a link or open a file. FILE answers link the filename to the admin-auth file stream route (`Content-Disposition: inline`, the same route the Excel export uses); URL text answers (`http(s)://` or bare `www.`) become external links; non-URL text (handles, numbers, Arabic) stays plain. `attendees/route.ts` now emits each FILE answer's `fileId` (`fileValues`) + the form column's `type`; the shared `dynamicColumnDef` renders file/URL/text — so group/phase columns get URL-linkify for free. No row-click conflict (only the Name cell is a `<Link>`; the `<tr>` has no onClick).
- **PR #68 (`54bd36f`) — per-event sequential attendee ID. ⚠️ SCHEMA CHANGE.** A stable per-event number (1, 2, 3, … within an event), shown as the **leftmost structural "ID" column** and the **first export column**, so an admin can track where they left off reviewing. `Contact.serialNumber Int?` + `@@unique([eventId, serialNumber])` (nullable, additive). **Race-safe allocation** via new `src/lib/attendees/serial.ts` `allocateContactSerial(tx, eventId)` — takes the event-row `SELECT … FOR UPDATE` lock (the `[[full-codebase-review-2026-06]]` capacity-lock pattern) and returns `max+1`. Wired into all three contact-creation paths: public register (already inside the locked tx), admin Add (wrapped in a tx), CSV/Excel import (**per-row tx** so one bad row still skips and the rest commit; the lock makes a serial clash impossible, so a P2002 there still only means duplicate email). **Only NEW contacts get a number; the register update path keeps the existing one** → an attendee's ID is stable across re-registration. Backfill `prisma/scripts/backfill-contact-serials.ts` — a **single bulk `ROW_NUMBER()` UPDATE per event** (rewrote a slow per-row loop that crawled on the **1,286-contact** event), idempotent + resume-safe (only touches NULLs, continues from each event's max). Maintainer ran `prisma db push --accept-data-loss` (the flag is **required for any new unique constraint** — non-destructive here: nullable column on all-NULL data) + the backfill against prod. **Deploy-gap gotcha (logged):** one straggler showed "—" — a contact registered in the window **between the prod backfill and the code deploy**, created by the still-live old code without a number. One more backfill run cleaned it; from deploy onward the new code numbers every contact. **Dead code:** `contactService.create/createMany` are unused, so weren't wired.
- **PR #69 (`9ea51c4`) — wider Name column.** Multi-word / Arabic names wrapped to 3 cramped lines; added `min-w-[220px]` to the Name header + cell (the table already has an `overflow-x-auto` wrapper, so no overflow).
- **PR #70 (`f70bd6a`) — filter by absence (Uncategorized / ungrouped / no-answer).** Closed the gap that you could filter by a value's **presence** but never its **absence** — across all three filter dimensions, via one shared sentinel `FILTER_NONE_VALUE` (new `src/lib/attendees/filter-constants.ts`, **server-import-free** so the client bundle can import it without pulling in prisma via `attendee-filters.ts`) handled in the **shared `buildContactWhere`** (so the screen AND the export both gain it): **Category** → new "Uncategorized" tab (`category` null/empty); **Groups** → "(None)" per group (`groupAssignments: { none: { groupId } }`); **Form answers** → "(No answer)" per field (no registration, or `""`/`[]` blank). CHECKBOX is always answered → deliberately no "no answer" option. Each filter ORs its real values with the absence case when both are picked (e.g. `Region = North OR (None)`). The Uncategorized sentinel is guarded so it never pre-fills a real category in the Add/Import dialogs. Verified with a controlled 4-contact dev event: **9/9** cases (existing value filters unchanged + the three new absence filters + combined). Reuses the same `[[attendee-field-filters]]` shared-where-builder contract.

**Noted gap (offered, not built):** FILE-upload fields aren't filterable at all today — can't filter "who hasn't uploaded a certificate/license yet." That's a new filter **dimension** (has-file / no-file), not another "none" option — the queued follow-up.

**Process note:** the favicon/OG PR (#65) and the export-dedup PR (#66) landed on the then-current `docs/handoff-2026-06-22` branch / fresh branches respectively; from #66 on, each change was branched off the latest `origin/main` (stash → `checkout -b … origin/main` → pop) to keep PR diffs clean after the prior squash-merges.
...
**⚠️ Known item — ✅ CLOSED 2026-06-28 by PR #66 (`725e0b6`):** a FormField mapped to a contact role but **named differently** (e.g. `company` → `mapsTo:ORGANIZATION`) showed as **two columns** — the base "Organization" column (resolved value) AND its own dynamic "Company" column (raw answer), same value duplicated. Cause: the dynamic-field exclusion (`CONTACT_COLUMN_NAMES`) was **name-based**, not mapping-based. The exact fix sketched here shipped — `isDynamicFormField` now also excludes any field with a non-null `mapsTo`. (Surfaced for real as duplicate First/Last Name in the productive-families export; see the 2026-06-28 entry at top.)
...
**Outstanding:** rotate the staging + production Neon passwords and the `branding-public` blob token (all surfaced in plaintext during setup). Minor: **save-over-old** branding blobs are now reclaimed by reconcile-on-replace (PR #81, 2026-06-29); the remaining orphan is the **upload-then-leave-without-Save** case (uploaded but never saved) — deliberately left unswept (KB-scale, no cron).
...
_Queue is empty — no queued substantive feature. **Per-Event Template System: infra shipped (Stage 1a, live, PR #83); template #2 PARKED by decision** (2026-06-29) pending a concrete design brief — a real event with a genuinely different visual structure to design against. Resume = mockup → build template #2, with the shared-engine factoring (Stage 1b) done + verified against it. See the top entry._

### DONE

- ✅ **Section Group Headers (v1)** — shipped 2026-06-29, PRs #74–#76. Polished section-header look on register page + portal phases, per-heading label color (form-builder picker → `FormField.metadata.color`, no schema change), §6 builder polish (distinct rows + swatch + "Add section heading"). Mechanism (`HEADING`) was already live; `FormField.section` confirmed dead. See the 2026-06-29 entry at top. v2 deferred: collapsible / conditional-empty-hide / descriptions.
- ✅ **Attendee-list QoL arc** — shipped 2026-06-28, PRs #65–#70. La Gloire favicon + OG link-preview (#65); export field-mapping dedup, closing the company→ORGANIZATION double-column (#66); clickable Website/Social + file links in the list (#67); **per-event sequential attendee ID** (`Contact.serialNumber` — additive schema, race-safe allocation under the event-row lock, idempotent bulk backfill `prisma/scripts/backfill-contact-serials.ts`) (#68); wider Name column (#69); **filter-by-absence** — Uncategorized / ungrouped / no-answer via one shared sentinel in `buildContactWhere` so screen==export (#70). See the 2026-06-28 entry at top.
...
- **FILE-field "has file / no file" filtering** (offered after PR #70, not started). FILE-upload fields aren't in `FILTERABLE_FIELD_TYPES`, so you can't filter "who hasn't uploaded a certificate/license yet." This is a new filter **dimension** (presence of an uploaded file), not another "(No answer)" option — would add FILE to `getFilterableFields` + a `RegistrationFile`/`formData`-path predicate in `buildContactWhere` (and decide the UI: a per-FILE-field has/no toggle). Useful given how much productive-families relies on uploads.
- ✅ **Friendlier admin file-op error surfacing — DONE 2026-06-29 (PR #80, `e42ffd1`).** A `preflightFileOp` GET to the existing read-back endpoint before `upload()` now surfaces the two reasons the SDK hid: expired session and the empty-field guard. (See the 2026-06-29 debt-items entry at top.)
