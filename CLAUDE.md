...
- **`PhaseAccess`** is a per-attendee override row (status `LOCKED` / `UNLOCKED` / `default`). Admins use it to grant/revoke access to specific phases for individual registrants without changing the phase's global open/close window.
